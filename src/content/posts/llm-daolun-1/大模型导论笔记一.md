...
## 现代 Decoder-only 模型还有 Cross-Attention 吗？

这里很容易产生一个误解：

> 既然 GPT、Llama、Qwen 等模型叫作 Decoder-only，那么它们的每一层是不是仍然包含原始 Transformer Decoder 里的 Cross-Attention？

答案是：**主流纯文本 Decoder-only 模型通常没有 Cross-Attention。** 这个误解混淆了两种不同架构中的“Decoder”。

### 原始 Transformer：Encoder-Decoder 架构

原始 Transformer 同时拥有 Encoder 和 Decoder。Encoder 先处理输入序列，Decoder 一边读取已经生成的目标序列，一边通过 Cross-Attention 查询 Encoder 的输出：

```text
输入序列 → Encoder → Encoder 输出 H_E
                         │
                         │ 提供 K、V
                         ▼
目标序列 → Decoder 的 Masked Self-Attention
                         ↓
                    Cross-Attention ← Decoder 提供 Q
                         ↓
                        FFN
```

因此，原始 Transformer 的 Decoder Layer 通常包含：

1. Masked（Causal）Self-Attention；
2. Cross-Attention；
3. FFN。

Cross-Attention 存在的理由非常直接：**Decoder 需要读取 Encoder 对输入序列的表示。** 这时两类注意力的 Q、K、V 来源分别是：

| 模块 | Q 的来源 | K、V 的来源 | 作用 |
|---|---|---|---|
| Decoder Self-Attention | Decoder 当前隐藏状态 | Decoder 当前隐藏状态 | 理解已经生成的目标序列 |
| Decoder Cross-Attention | Decoder 当前隐藏状态 | Encoder 最终输出 | 从输入序列中检索相关信息 |

### 现代主流 LLM：Decoder-only 架构

GPT、Llama、Qwen 等主流纯文本大模型去掉了整个 Encoder，也就没有一份独立的 Encoder 输出可供 Cross-Attention 查询。它们把提示词和已经生成的内容放在**同一个 token 序列**中，通过因果自注意力统一处理：

```text
Token Embedding
      ↓
Decoder Layer 1：Causal Self-Attention + MLP/FFN
      ↓
Decoder Layer 2：Causal Self-Attention + MLP/FFN
      ↓
     ...
      ↓
Decoder Layer L
...
所以现代 Decoder-only 模型的核心 Decoder Layer 可以概括为：

<div align="center">

$$ \boxed{\text{Causal Self-Attention}+\text{MLP/FFN}} $$

</div>

其中当然还会配合归一化、残差连接等结构，但**不会因为名字中有 Decoder，就必然带有 Cross-Attention**。

每一层接收上一层输出的隐藏状态 $H^{(l-1)}$，再用本层自己的参数重新计算 Q、K、V：

<div align="center">

$$ Q^{(l)}=H^{(l-1)}W_Q^{(l)},\qquad K^{(l)}=H^{(l-1)}W_K^{(l)},\qquad V^{(l)}=H^{(l-1)}W_V^{(l)} $$

</div>

经过 Causal Self-Attention 和 MLP 后得到 $H^{(l)}$，再交给下一层。层与层之间传递的是更新后的**隐藏状态**，不是上一层的 Q、K、V 或注意力分数。

### 三个容易混淆的概念

- **Self / Cross** 描述 Q、K、V 是否来自同一序列；
- **Causal** 描述是否用掩码阻止当前位置看到未来 token；
- **Multi-Head** 描述是否并行使用多组投影，从多个表示子空间学习 token 关系。

所以现代 Decoder-only LLM 通常使用的是**多头因果自注意力**：它是 Self-Attention，也是 Causal Attention，同时还是 Multi-Head Attention，但它不是 Cross-Attention。

> 补充：某些多模态或带外部编码器的模型会专门加入 Cross-Attention 来读取图像、音频等表示，但这是具体模型额外设计的连接，并非纯文本 Decoder-only LLM 的默认结构。
