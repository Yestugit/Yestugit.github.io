...
- **Harness** 是较宽的运行与治理层，关注“Agent 应该怎样工作、如何保持可靠”；
- **Agent Runtime** 是其中的执行基础设施，关注“模型调用、工具和状态怎样真正跑起来”。

![Harness 与 Agent Runtime 的职责边界：Harness 负责控制和治理，Runtime 提供执行基础](./harness-runtime-boundary.svg)

可以把 Harness 类比成一个受控工作系统，把 Runtime 类比成这个系统的执行底座：

| 维度 | Harness | Agent Runtime |
|---|---|---|
| 核心问题 | 应该做什么，怎样可靠地推进？ | 在哪里执行，靠什么把动作跑起来？ |
...
## 五、先理顺一个 token 的“时间错位”，再理解 KV Cache

缓存问题里最容易卡住的一句话是：“每生成一个新 token，它的 Query 都要与前面 token 的 Key 匹配。”这句话如果不说明时间顺序，很容易让人困惑：token 还没有生成，怎么已经有它的 Query？

关键是区分**当前 token 的表示计算**和**下一个 token 的预测**。假设当前序列是：

```text
我  喜欢  吃
```

模型使用“吃”这个位置经过多层 Attention 后的最终隐藏状态，得到词表概率并预测出“苹果”：

```text
h(吃) → LM Head → logits → Softmax → 采样得到“苹果”
```

“苹果”生成后，才进入下一轮解码。此时模型计算“苹果”在每一层的 Q、K、V，让它关注“我、喜欢、吃、苹果”，构造 `h(苹果)`，用于预测**苹果后面的 token**。

> **第 t 个位置的最终隐藏状态预测第 t+1 个 token；第 t+1 个 token 被选出后，再计算它自己的 Q、K、V，用于预测第 t+2 个 token。**

这也说明 Q、K、V 并不直接输出词表概率。它们只是每层 Self-Attention 的中间计算，负责让当前 token 融合前文信息；最后一层隐藏状态还要经过 LM Head 和 Softmax，才得到下一个 token 的概率分布。

## 六、KV Cache 与 Prompt Cache：同一种复用思想，两个作用范围

当“苹果”进入下一轮时，它的 Query 要与此前所有 token 的 Key 匹配，并按权重汇总 Value。历史 token 的 K、V 已经算过，而且在同一轮自回归生成中不会改变，因此可以直接缓存，而不必每生成一个 token 就从头重算整个前缀。
...
## 七、上下文感知压缩：不是摘要得更短，而是只保留当前任务需要的新信息
...
2. **Harness 是较宽的运行与治理层，Runtime 是让模型、工具和状态真正运行的执行基础。**
